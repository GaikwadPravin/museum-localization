...
def show(window, button, act_sys_nu, entries_x, entries_y):
    print("Inside show")
    ...
    if(x_co > x2_cord):
        x_co = x2_cord - 50
        messagebox.showerror("Error", "x position is too large for canvas")
    if(y_co > y2_cord):
        y_co = y2_cord - 50
        messagebox.showerror("Error", "y position is too large for canvas")
    button_name = button["text"]
    button_nu = int(button_name.split(' ')[-1]) - 1
    ...
    except_flag = 0
    try:
        play_manual[button_nu].place_forget()
        print("try")
    except:
        play_manual[button_nu].place(x=x_co, y=y_co)
        except_flag = 1
        print("except")
    if(except_flag == 0):
        play_manual[button_nu].place(x=x_co, y=y_co)
...
def manual_source(button):
    ...
    #messagebox.showerror("Error", "Error message")
        ...
        show_b = Button(window, text='Show', command=lambda:show(window, button, act_sys_nu,  entries_x, entries_y))
...
for i in range(15, 0, -1):
    ...
    lbuttons.append(Button(leftFrame,command=lambda temp_j=temp_j:browse_for_file(lbuttons[temp_j], buttons_upload, lbuttons_x, files, llabels, leftFrame, labels_play, load_src_b, buttons_play) ,text="Upload File "+ str(i), fg="white", bg = "dim gray", highlightbackground="black", relief="flat"))
    ...
    play_manual.append(Button(midFrame, text = str(temp_j+1), command=lambda:play_sounds(lbuttons[temp_j], files), anchor = W, image = play, relief = FLAT, bg="white",highlightbackground="white"))
    play_manual[15-i].config(compound="bottom")

for i in range(0,15):
    buttons_play.append(Button(midFrame, text = str(15-i),command=lambda i=i:play_sounds(buttons_play[i]), anchor = W, image = play, bg="white", relief = FLAT, highlightbackground="white"))
...
load_src_b = Button(leftFrame,command=lambda temp_j=temp_j:default_source(y2_cord) ,text="Load GUI(A)", fg="white", bg = "dim gray", highlightbackground="black", relief="flat", state=DISABLED, width = 8)
...
rbutton5 = Button(rightFrame, command=lambda:default_sys(midFrame, lbuttons_x, buttons_upload, files, llabels, load_src_b, buttons_play, labels_play, play, y2_cord),text="Default",fg="white", bg = "dim gray", highlightbackground="black", relief="flat", width=9)
