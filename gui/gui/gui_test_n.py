...
def show(window, entries_x, entries_y):
    x_co = int(entries_x.get())
    y_co = int(entries_y.get())
    Button(midFrame, text = "Play "+str(i),command=lambda i=i:play_sounds(buttons_play[i]), anchor = W, image = play, relief = FLAT, bg="white",highlightbackground="white").place(x=x_co, y=y_co)
    #buttons_play[-1].configure(width = 50,height = 34, activebackground = "#33B5E5", relief = FLAT, highlightbackground="white")
    #button_window = midCanvas.create_window(15, 15*(i*3+1), anchor=NW, window=buttons_play[i])
'''
def configure(event):
   midCanvas.delete()
   w, h = event.width, event.height
   #midCanvas_new = Canvas(midFrame, width=w, height=h, bg = "white")
   xy = 0, 0, w-1, h-1
   midCanvas.configure(width=w,height=h)
   #midCanvas.create_rectangle(xy)
   #midCanvas.create_line(xy)
   #xy = w-1, 0, 0, h-1
'''
...
def manual_source(button):
    ...
    if(files[button_nu] is not None):
    #messagebox.showerror("Error", "Error message")
        ...
        entries_x.grid(row=2,column=1)
        entries_y = Entry(window)
        entries_y.grid(row=2,column=2)

        show_b = Button(window, text='Show', command=lambda:show(window, entries_x, entries_y))
...
for i in range(15, 0, -1):
    ...
    lbuttons.append(Button(leftFrame,command=lambda temp_j=temp_j:browse_for_file(lbuttons[temp_j], files, llabels, leftFrame, labels_play, load_src_b, load_src_b_m, buttons_play) ,text="Upload File "+ str(i), fg="white", bg = "dim gray", highlightbackground="black", relief="flat"))
    ...
    buttons_upload.append(Button(leftFrame, command=lambda temp_j=temp_j:manual_source(buttons_play[temp_j]), anchor = W, text="U", relief = FLAT, fg="white", bg="dim gray",highlightbackground="black"))
